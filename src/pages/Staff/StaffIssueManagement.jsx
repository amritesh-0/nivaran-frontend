...
import { allIssuesData, STAFF_ID } from '../../data/mockData.js';
import { getStatusPill, getSeverityPill } from '../../utils/styleUtils.jsx';

const StaffIssueManagement = () => {
  const [searchTerm, setSearchTerm] = useState('');
  const [filters, setFilters] = useState({ status: 'all', category: 'all', severity: 'all' });

  // Filter issues to only show those assigned to the staff member
  const myIssues = allIssuesData.filter(issue => issue.assigned === STAFF_ID);
          ...
          {['status', 'category', 'severity'].map((filterName) => (
            <div key={filterName} className="relative">
              ...
              >
                ...
                {[...new Set(myIssues.map((item) => item[filterName]))].map((option) => (
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                {['Issue', 'Category', 'Severity', 'Status', 'Date', ''].map((header) => (
...
export default StaffIssueManagement;
